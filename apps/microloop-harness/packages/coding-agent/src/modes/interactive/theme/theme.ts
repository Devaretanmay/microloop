...
function getBuiltinThemes(): Record<string, ThemeJson> {
	if (!BUILTIN_THEMES) {
		...
		BUILTIN_THEMES = {
			...
			"microloop-dark": JSON.parse(fs.readFileSync(path.join(themesDir, "microloop-dark.json"), "utf-8")) as ThemeJson,
			"microloop-light": JSON.parse(fs.readFileSync(path.join(themesDir, "microloop-light.json"), "utf-8")) as ThemeJson,
		};
	}
...
export function getDefaultTheme(): string {
	...
	return terminalTheme === "light" ? "microloop-light" : "microloop-dark";
}
...
const THEME_KEY = Symbol.for("@microloop/coding-agent:theme");
