...
import { type ImageContent, modelsAreEqual } from "@microloop/ai";
...
import { handleConfigCommand, handleLearnCommand, handlePackageCommand } from "./package-manager-cli.ts";
...
export async function main(args: string[], options?: MainOptions) {
	resetTimings();
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.MICROLOOP_OFFLINE);
	if (offlineMode) {
	...
	if (await handleLearnCommand(args)) {
	...
	if (appMode === "interactive" && !parsed.help && parsed.listModels === undefined && shouldRunFirstTimeSetup()) {
		...
		// After first-time setup, prompt for API key if none configured
		const authStorage = AuthStorage.create();
		if (authStorage.list().length === 0) {
			await showProviderSetup(startupSettingsManager);
		}
		time("firstTimeSetup");
	}
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.MICROLOOP_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: MICROLOOP_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
