...
import { Markdown, type MarkdownTheme } from "@microloop/tui";
...
import { getLatestMicroloopRelease, isNewerPackageVersion } from "./utils/version-check.ts";
import {
...
/**
 * Handle the "learn" subcommand: offline trajectory analyzer.
 * Parses an exported .jsonl session file via the microloop-learn binary.
 */
...
function parseLearnCommand(args: string[]): LearnCommandOptions | undefined {
	const [command] = args;
	if (command !== "learn") {
		return undefined;
	}
	...
	for (let i = 1; i < args.length; i++) {
		...
		if (arg === "-h" || arg === "--help") {
			...
			console.error(chalk.red(`Unknown option ${arg} for "learn".`));
			console.error(chalk.dim(`Use "${APP_NAME} learn --help" for usage.`));
			process.exitCode = 1;
			return undefined;
		}
	}

	return { input, summarizeOnly, help };
}

export async function handleLearnCommand(
	args: string[],
): Promise<boolean> {
	const options = parseLearnCommand(args);
	if (!options) {
		return false;
	}
	...
	if (options.help) {
		console.log(`${chalk.bold("Usage:")}
  ${APP_NAME} learn --input <file> [--summarize|-s]

Analyze an exported session .jsonl file via microloop-learn to detect loops
and generate a safety policy.

Options:
  --input <file>        Path to a session .jsonl file (required)
  -s, --summarize       Print a behavioral summary instead of generating a policy
  -h, --help            Show this help
`);
		return true;
	}

	// Resolve the microloop-learn binary path
	...
	if (!options.input) {
		console.error(chalk.red("Missing required option: --input <file>"));
		console.error(chalk.dim(`Usage: ${APP_NAME} learn --input <file>`));
		process.exitCode = 1;
		return true;
	}
	...
	try {
		...
		console.error(chalk.red(`Could not access input file: ${options.input}`));
		process.exitCode = 1;
		return true;
	}

	// Build args for microloop-learn
	const learnArgs = [`--input`, options.input];
	if (options.summarizeOnly) {
		learnArgs.push("--summarize");
	}

	try {
		const result = await new Promise<string>((resolve, reject) => {
			const child = spawnProcess(learnPath, learnArgs, {
			...
			child.on("close", (code: number | null, signal: string | null) => {
				if (code === 0) {
					...
					reject(new Error(`microloop-learn terminated by signal ${signal}`));
				} else if (stderr) {
					reject(new Error(stderr.trim()));
				} else {
					reject(new Error(`microloop-learn exited with code ${code ?? "unknown"}`));
				}
			});
		});
		...
		console.error(chalk.red(`microloop-learn error: ${message}`));
		process.exitCode = 1;
		return true;
	}
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	let latestRelease: Awaited<ReturnType<typeof getLatestMicroloopRelease>>;
	try {
		latestRelease = await getLatestMicroloopRelease(VERSION);
	} catch (error: unknown) {
		const message = error instanceof Error ? error.message : String(error);
