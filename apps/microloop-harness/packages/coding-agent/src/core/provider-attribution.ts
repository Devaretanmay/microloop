import type { Api, Model, ProviderHeaders } from "@microloop/ai";
...
function getDefaultAttributionHeaders(
	...
	if (isOpenRouterModel(model)) {
		return {
			"HTTP-Referer": "https://microloop.dev",
			"X-OpenRouter-Title": "microloop",
	...
	if (isNvidiaNimModel(model)) {
		return {
			"X-BILLING-INVOKE-ORIGIN": "Microloop",
		};
	}

	if (isCloudflareModel(model)) {
		return {
			"User-Agent": "microloop",
		};
	}
...
function getSessionHeaders(model: Model<Api>, sessionId: string | undefined): Record<string, string> | undefined {
	...
	return { "x-opencode-session": sessionId, "x-opencode-client": "microloop" };
}
