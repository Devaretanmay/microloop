...
function isOfflineModeEnabled(): boolean {
	const value = process.env.MICROLOOP_OFFLINE;
...
function readPackageManifestFile(packageJsonPath: string): MicroloopManifest | null {
	try {
		const content = readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content) as { microloop?: MicroloopManifest };
		return pkg.microloop ?? null;
	} catch {
		return null;
	}
}
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// @microloop/* peers. Stale auto-installed pi peers can otherwise block updates.
	...
	private collectPackageResources(
		...
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof MicroloopManifest];
	...
	private readPackageManifest(packageRoot: string): MicroloopManifest | null {
