...
import { getModel } from "@microloop/ai/compat";
...
describe("AgentSession dynamic provider registration", () => {
	...
	it("applies top-level registerProvider overrides to the active model", async () => {
		const session = await createSession([
			(microloop) => {
				microloop.registerProvider("anthropic", { baseUrl: "http://localhost:8080/top-level" });
			},
		]);
	...
	it("applies session_start registerProvider overrides to the active model", async () => {
		const session = await createSession([
			(microloop) => {
				microloop.on("session_start", () => {
					microloop.registerProvider("anthropic", { baseUrl: "http://localhost:8080/session-start" });
				});
			},
		]);
	...
	it("applies command-time registerProvider overrides without reload", async () => {
		const session = await createSession([
			(microloop) => {
				microloop.registerCommand("use-proxy", {
					description: "Use proxy",
					handler: async () => {
						microloop.registerProvider("anthropic", { baseUrl: "http://localhost:8080/command" });
					},
				});
			},
		]);
