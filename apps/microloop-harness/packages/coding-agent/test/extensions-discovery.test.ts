...
describe("extensions discovery", () => {
	...
	const extensionCode = `
		export default function(microloop) {
			microloop.registerCommand("test", { handler: async () => {} });
		}
	`;
	...
	it("discovers direct .ts files in extensions/", async () => {
		...
		console.log("ERRORS:", JSON.stringify(result.errors)); expect(result.errors).toHaveLength(0);
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(path.join(subdir2, "package.json"), JSON.stringify({ microloop: { extensions: ["./entry.ts"] } }));
	...
	it.skip("resolves dependencies from extension's own node_modules", async () => {
	...
	it("registers message and entry renderers", async () => {
		const extCode = `
			export default function(microloop) {
				microloop.registerMessageRenderer("my-custom-type", (message, options, theme) => {
					return null; // Use default rendering
				});
				microloop.registerEntryRenderer("my-entry-type", (entry, options, theme) => {
					return null;
				});
			}
		`;
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(microloop) {
				microloop.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(microloop) {
				microloop.on("agent_start", async () => {});
				microloop.on("tool_call", async (event) => undefined);
				microloop.on("agent_end", async () => {});
			}
		`;
	...
	it("loads extension with shortcuts", async () => {
		const extCode = `
			export default function(microloop) {
				microloop.registerShortcut("ctrl+t", {
