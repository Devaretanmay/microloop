...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".microloop", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".microloop", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".microloop", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".microloop"), { recursive: true });
			writeFileSync(join(cwd, ".microloop", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".microloop"), { recursive: true, force: true });
