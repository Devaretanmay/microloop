...
describe("ExtensionRunner", () => {
	...
	microloop.on("project_trust", () => ({ trusted: "undecided", remember: true }));
}`,
			);
			fs.writeFileSync(
				decidedPath,
				`export default function(microloop) {
	microloop.on("project_trust", () => ({ trusted: "no", remember: true }));
}`,
			);
	...
	describe("shortcut conflicts", () => {
		it("warns when extension shortcut conflicts with built-in", async () => {
			const extCode = `
				export default function(microloop) {
					microloop.registerShortcut("ctrl+c", {
		...
		it("allows a shortcut when the reserved set no longer contains the default key", async () => {
			const extCode = `
				export default function(microloop) {
					microloop.registerShortcut("ctrl+p", {
		...
		it("warns but allows when extension uses non-reserved built-in shortcut", async () => {
			...
			const extCode = `
				export default function(microloop) {
					microloop.registerShortcut("${pasteImageKey}", {
		...
		it("blocks shortcuts for reserved actions even when rebound", async () => {
			const extCode = `
				export default function(microloop) {
					microloop.registerShortcut("ctrl+x", {
		...
		it("blocks shortcuts when reserved action has multiple keys", async () => {
			const extCode = `
				export default function(microloop) {
					microloop.registerShortcut("ctrl+y", {
		...
		it("warns when two extensions register same shortcut", async () => {
			...
			const extCode1 = `
				export default function(microloop) {
					microloop.registerShortcut("ctrl+shift+x", {
	...
	describe("command collection", () => {
		it("collects commands from multiple extensions", async () => {
			const cmdCode = (name: string) => `
				export default function(microloop) {
					microloop.registerCommand("${name}", {
		...
		it("gets command by invocation name", async () => {
			const cmdCode = `
				export default function(microloop) {
					microloop.registerCommand("my-cmd", {
		...
		it("suffixes duplicate extension commands in insertion order", async () => {
			const cmdCode = (description: string) => `
				export default function(microloop) {
					microloop.registerCommand("shared-cmd", {
	...
	describe("message and entry renderers", () => {
		it("gets message renderer by type", async () => {
			const extCode = `
				export default function(microloop) {
					microloop.registerMessageRenderer("my-type", (message, options, theme) => null);
				}
			`;
		...
		it("gets entry renderer by type", async () => {
			const extCode = `
				export default function(microloop) {
					microloop.registerEntryRenderer("my-entry", (entry, options, theme) => null);
				}
			`;
	...
	describe("flags", () => {
		it("collects flags from extensions", async () => {
			const extCode = `
				export default function(microloop) {
					microloop.registerFlag("my-flag", {
						description: "My flag",
						handler: async () => {},
					});
				}
		...
		it("keeps first flag when two extensions register the same name", async () => {
			const first = `
				export default function(microloop) {
					microloop.registerFlag("shared-flag", {
						description: "first",
						type: "boolean",
						default: true,
					});
		...
		it("can set flag values", async () => {
			const extCode = `
				export default function(microloop) {
					microloop.registerFlag("test-flag", {
	...
	describe("before_agent_start", () => {
		it("keeps ctx.getSystemPrompt() in sync with chained system prompt updates", async () => {
			const extCode1 = `
				export default function(microloop) {
					microloop.on("before_agent_start", async (_event, ctx) => {
						return {
	...
	describe("tool_result chaining", () => {
		it("chains content modifications across handlers", async () => {
			const extCode1 = `
				export default function(microloop) {
					microloop.on("tool_result", async (event) => {
						return {
		...
		it("preserves previous modifications when later handlers return partial patches", async () => {
			const extCode1 = `
				export default function(microloop) {
					microloop.on("tool_result", async () => {
						return {
	...
	describe("hasHandlers", () => {
		it("returns true when handlers exist for event type", async () => {
			const extCode = `
				export default function(microloop) {
					microloop.on("tool_call", async () => undefined);
				}
			`;
	...
	describe("before_provider_headers", () => {
		it("lets a handler mutate headers in place and preserves existing headers", async () => {
			const extCode = `
				export default function(microloop) {
					microloop.on("before_provider_headers", (event) => {
		...
		it("isolates a throwing handler and still applies the others", async () => {
			const throwing = `
				export default function(microloop) {
					microloop.on("before_provider_headers", () => {
