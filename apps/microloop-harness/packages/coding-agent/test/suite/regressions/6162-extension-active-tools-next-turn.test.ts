import { fauxAssistantMessage, fauxToolCall } from "@microloop/ai";
...
describe("extension active tools next-turn refresh", () => {
	it("applies microloop.setActiveTools before the next provider request in the same run", async () => {
		const extensionFactories: ExtensionFactory[] = [
			(microloop) => {
				microloop.registerTool({
					...
					execute: async () => {
						microloop.setActiveTools(["after_switch"]);
						return {
		...
		const extensionFactories: ExtensionFactory[] = [
			(microloop) => {
				microloop.registerTool({
					...
					parameters: Type.Object({}),
					execute: async () => {
						microloop.setActiveTools([...microloop.getActiveTools(), "after_load"]);
						return {
