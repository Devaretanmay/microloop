...
describe("regression #6363: agent settled event and idle waiting", () => {
	...
	it("emits one agent_settled event after automatic retry finishes", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(microloop) => {
					microloop.on("agent_end", () => {
						extensionEvents.push("agent_end");
					});
					microloop.on("agent_settled", (_event, ctx) => {
			...
			extensionFactories: [
				(microloop) => {
					microloop.on("agent_end", () => {
						...
						queuedFollowUp = true;
						microloop.sendUserMessage("status follow-up", { deliverAs: "followUp" });
					});
					microloop.on("agent_settled", (_event, ctx) => {
	...
	it("extension command waitForIdle waits for session-level settlement", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(microloop) => {
					microloop.registerCommand("after-idle", {
