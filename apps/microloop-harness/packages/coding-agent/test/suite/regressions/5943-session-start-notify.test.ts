import { fauxAssistantMessage } from "@microloop/ai";
import { Container, Text } from "@microloop/tui";
...
describe("regression #5943: session_start transient UI", () => {
	...
	it("renders replacement session state before session_start handlers can notify", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(microloop) => {
					microloop.on("session_start", (_event, ctx) => {
	...
	it("subscribes before replacement session_start handlers send user messages", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(microloop) => {
					microloop.on("session_start", () => {
						microloop.sendUserMessage("user from start");
					});
				},
			],
		});
	...
	it("runs the reload render hook before reload session_start handlers can notify", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(microloop) => {
					microloop.on("session_start", (event, ctx) => {
