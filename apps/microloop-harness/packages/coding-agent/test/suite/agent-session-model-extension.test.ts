import type { AgentTool, ThinkingLevel } from "@microloop/agent";
import { fauxAssistantMessage, fauxToolCall, type Model } from "@microloop/ai";
...
describe("AgentSession model and extension characterization", () => {
	...
	it("setModel saves the model and emits model_select", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(microloop) => {
					microloop.on("model_select", async (event) => {
	...
	it("allows extension tool_call handlers to block tool execution", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(microloop) => {
					microloop.on("tool_call", async () => ({ block: true, reason: "Blocked by test" }));
				},
			],
		});
		harnesses.push(harness);
	...
	it("allows extension context handlers to modify messages before the LLM call", async () => {
		const harness = await createHarness({
			extensionFactories: [
				(microloop) => {
					microloop.on("context", async (event) => ({
	...
	it("allows extension input handlers to transform or handle input", async () => {
		...
		const transformedHarness = await createHarness({
			extensionFactories: [
				(microloop) => {
					extensionApi = microloop;
					microloop.on("input", async (event) => {
						if (event.text === "ping") {
	...
	it("allows extension commands to inspect live system prompt options", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(microloop) => {
					microloop.registerCommand("inspect-options", {
	...
	it("allows before_agent_start handlers to inject custom messages and modify the system prompt", async () => {
		const harness = await createHarness({
			extensionFactories: [
				(microloop) => {
					microloop.on("before_agent_start", async (event) => ({
						message: {
	...
	it("bindExtensions emits session_start and reload emits session_shutdown then session_start", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(microloop) => {
					microloop.on("session_start", async (event) => {
						lifecycleEvents.push(`start:${event.reason}`);
					});
					microloop.on("session_shutdown", async (event) => {
