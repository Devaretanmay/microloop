...
import { fauxAssistantMessage, registerFauxProvider } from "@microloop/ai/compat";
...
describe("AgentSessionRuntime characterization", () => {
	...
	async function createRuntimeForTest(
		...
		const runtimeOptions = {
			...
			resourceLoaderOptions: {
				extensionFactories: [
					(microloop: ExtensionAPI) => {
						microloop.registerProvider(faux.getModel().provider, {
	...
	it("persists message_end assistant replacements to the session manager", async () => {
		const { runtime } = await createRuntimeForTest((microloop: ExtensionAPI) => {
			microloop.on("message_end", (event) => {
	...
	it("emits session_before_switch and session_start for new and resume flows", async () => {
		...
		const { runtime } = await createRuntimeForTest((microloop: ExtensionAPI) => {
			microloop.on("session_before_switch", (event) => {
				events.push(event);
			});
			microloop.on("session_shutdown", (event) => {
				events.push(event);
			});
			microloop.on("session_start", (event) => {
				events.push(event);
			});
		});
	...
	it("emits session_before_fork and session_start and honors cancellation", async () => {
		...
		const { runtime } = await createRuntimeForTest((microloop: ExtensionAPI) => {
			microloop.on("session_before_fork", (event) => {
				events.push(event);
				if (cancelNextFork) {
