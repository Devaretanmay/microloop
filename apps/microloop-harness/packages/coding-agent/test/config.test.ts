...
const originalPiPackageDir = process.env.MICROLOOP_PACKAGE_DIR;
...
afterEach(() => {
	...
	if (originalPiPackageDir === undefined) {
		delete process.env.MICROLOOP_PACKAGE_DIR;
	} else {
		process.env.MICROLOOP_PACKAGE_DIR = originalPiPackageDir;
	}
...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	tempDir = prefix;
	process.env.MICROLOOP_PACKAGE_DIR = packageDir;
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@microloop/coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @microloop/coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@microloop/coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@microloop/coding-agent")).toBe(
			"Update @microloop/coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@microloop/coding-agent");
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@microloop/coding-agent",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @microloop/coding-agent`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@microloop/coding-agent", undefined, {
			packageName: "@microloop/coding-agent",
			installSpec: "@microloop/coding-agent@1.2.3",
		});
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@microloop/coding-agent@1.2.3",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @microloop/coding-agent@1.2.3`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@microloop/coding-agent", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @microloop/coding-agent`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@microloop/coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @microloop/coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@microloop/coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @microloop/coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@microloop/coding-agent";
		...
		tempDir = temp;
		process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
		process.env.MICROLOOP_PACKAGE_DIR = storePackageDir;
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@microloop/coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@microloop/coding-agent")).toContain(
