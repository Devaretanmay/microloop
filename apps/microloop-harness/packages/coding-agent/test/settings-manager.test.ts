...
describe("SettingsManager", () => {
	...
	beforeEach(() => {
		...
		mkdirSync(join(projectDir, ".microloop"), { recursive: true });
	});
	...
	describe("error tracking", () => {
		it("should collect and clear load errors via drainErrors", () => {
			...
			const projectSettingsPath = join(projectDir, ".microloop", "settings.json");
	...
	describe("project trust", () => {
		it("should skip project settings when project is not trusted", () => {
			...
			writeFileSync(join(projectDir, ".microloop", "settings.json"), JSON.stringify({ theme: "project" }));
		...
		it("should read default project trust from global settings only", () => {
			...
			writeFileSync(join(projectDir, ".microloop", "settings.json"), JSON.stringify({ defaultProjectTrust: "never" }));
	...
	describe("project settings directory creation", () => {
		it("should not create .microloop folder when only reading project settings", () => {
			// Create agent dir with global settings, but NO .microloop folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the .microloop folder that beforeEach created
			rmSync(join(projectDir, ".microloop"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// .microloop folder should NOT have been created just from reading
			expect(existsSync(join(projectDir, ".microloop"))).toBe(false);

			// Settings should still be loaded from global
			expect(manager.getTheme()).toBe("dark");
		});

		it("should create .microloop folder when writing project settings", async () => {
			// Create agent dir with global settings, but NO .microloop folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// .microloop folder should NOT exist yet
			expect(existsSync(join(projectDir, ".microloop"))).toBe(false);
			...
			// Now .microloop folder should exist
			expect(existsSync(join(projectDir, ".microloop"))).toBe(true);

			// And settings file should be created
			expect(existsSync(join(projectDir, ".microloop", "settings.json"))).toBe(true);
		});
	});
	...
	describe("httpIdleTimeoutMs", () => {
		...
		it("should use merged global and project settings", () => {
			...
			writeFileSync(join(projectDir, ".microloop", "settings.json"), JSON.stringify({ httpIdleTimeoutMs: 0 }));
	...
	describe("getSessionDir", () => {
		...
		it("should return project sessionDir, overriding global", () => {
			...
			writeFileSync(join(projectDir, ".microloop", "settings.json"), JSON.stringify({ sessionDir: "./sessions" }));
			const manager = SettingsManager.create(projectDir, agentDir);
