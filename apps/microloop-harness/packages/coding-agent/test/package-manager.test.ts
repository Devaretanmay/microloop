...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.MICROLOOP_OFFLINE;
		delete process.env.MICROLOOP_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.MICROLOOP_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .microloop skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .microloop", async () => {
			const extDir = join(tempDir, ".microloop", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".microloop"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".microloop", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".microloop", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".microloop", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".microloop", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".microloop", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json microloop.extensions in extensions setting", async () => {
			// Create a package with microloop.extensions in package.json
			...
			// Should find the extensions declared in package.json microloop.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .microloop/agent skills", async () => {
		...
		it("should use the project .microloop dir as baseDir for project .microloop skills", async () => {
			const projectBaseDir = join(tempDir, ".microloop");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".microloop", "agent");
		...
		it("should dedupe user skill entries when ~/.microloop/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .microloop auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".microloop\n");

			const skillDir = join(tempDir, ".microloop", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.microloop\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".microloop", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .microloop settings base", () => {
			...
			const rel = relative(join(tempDir, ".microloop"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{ source: relative(join(tempDir, ".microloop"), pkgDir), autoload: false, extensions: ["+extensions/foo.ts"] },
			]);
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json microloop.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".microloop", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".microloop", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".microloop", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".microloop", "npm", "node_modules", "project-current");
		...
		it("should skip installing missing package sources when offline", async () => {
			process.env.MICROLOOP_OFFLINE = "1";
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".microloop", "npm", "node_modules", "example");
