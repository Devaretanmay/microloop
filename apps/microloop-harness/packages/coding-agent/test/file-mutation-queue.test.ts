...
describe("withFileMutationQueue", () => {
	...
	it("keeps edit queue locked while an aborted edit write is still in flight", async () => {
		...
		// Second edit should wait for first to settle
		const secondEdit = editTool.execute("call-2", {
			path: filePath,
			edits: [{ oldText: "beta", newText: "BETA" }],
		});

		// Abort first edit while it's in flight
		controller.abort();
		await expect(firstEdit).rejects.toThrow();
		await secondEdit;

		const content = await readFile(filePath, "utf8");
		// Second edit should apply (first was aborted before writing)
		expect(content).toBe("alpha\nBETA\n");
	});
});
