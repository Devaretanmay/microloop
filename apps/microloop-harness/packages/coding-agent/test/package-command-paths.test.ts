...
describe("package commands", () => {
	...
	beforeEach(() => {
		...
		originalPiPackageDir = process.env.MICROLOOP_PACKAGE_DIR;
	...
	it("skips untrusted project package settings", async () => {
		...
		writeFileSync(join(projectDir, ".microloop", "settings.json"), JSON.stringify({ packages: ["npm:@project/pkg"] }));
		...
		try {
			await expect(
				main(["list"], {
					extensionFactories: [
						(microloop) => {
							microloop.on("project_trust", () => ({ trusted: "yes" }));
						},
					],
				}),
	...
	it("does not prompt or ask extensions for project trust during update", async () => {
		...
		writeFileSync(
			join(projectDir, ".microloop", "settings.json"),
	...
	it("blocks local package changes when project is untrusted", async () => {
		mkdirSync(join(projectDir, ".microloop"), { recursive: true });
		writeFileSync(join(projectDir, ".microloop", "settings.json"), "{}");
	...
	it("allows local package install to initialize fresh project settings", async () => {
		...
		const settingsPath = join(projectDir, ".microloop", "settings.json");
		const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
		expect(settings.packages?.length).toBe(1);
		const stored = settings.packages?.[0] ?? "";
		expect(realpathSync(join(projectDir, ".microloop", stored))).toBe(realpathSync(packageDir));
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("microloop install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "microloop --help" or "microloop install <source> [-l] [--approve|--no-approve]".');
			expect(process.exitCode).toBe(1);
		} finally {
			errorSpy.mockRestore();
		}
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: microloop install <source> [-l]");
...
`,
		...
		process.env.MICROLOOP_PACKAGE_DIR = selfPackageDir;
		...
		try {
			...
			expect(stdout).toContain(`Updated microloop from ${VERSION} to ${VERSION}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
		...
		try {
			...
			expect(recordedArgs).not.toContain(PACKAGE_NAME);
			expect(stdout).toContain(`Updated microloop from ${VERSION} to ${targetVersion}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
	...
	it("prints a pnpm metadata hint when self-update fails", async () => {
		...
		try {
			...
			expect(stderr).toContain("Run `pnpm store prune` and retry `microloop update --self`.");
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
