...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".microloop", "prompts");
			...
			const projectSkillDir = join(cwd, ".microloop", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".microloop", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".microloop", "themes"), { recursive: true });
	...
	microloop.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".microloop"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".microloop", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".microloop", "extensions", "shared.ts"));
		});

		it("should load user extensions before trust and reuse them after trust resolves", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".microloop", "extensions");
...
export default function(microloop) {
	...
	microloop.registerCommand("user-trust", {
		...
		handler: async () => {},
			...
			writeFileSync(
				...
				`export default function(microloop) {
	microloop.registerCommand("project-trusted", {
		...
		handler: async () => {},
			...
			const extensionsResult = loader.getExtensions();
			expect(extensionsResult.extensions.map((extension) => extension.path)).toEqual([
				join(cwd, ".microloop", "extensions", "project.ts"),
				join(userExtDir, "user.ts"),
			]);
		...
		it("should keep both extensions loaded when command names collide", async () => {
			...
			writeFileSync(
				join(projectExtDir, "project.ts"),
				`export default function(microloop) {
	microloop.registerCommand("deploy", {
		description: "project deploy",
		handler: async () => {},
	});
	microloop.registerCommand("project-only", {
	...
	microloop.registerCommand("user-only", {
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.microloop", async () => {
			const piDir = join(cwd, ".microloop");
...
export default function(microloop: ExtensionAPI) {
  microloop.registerTool({
    name: "duplicate-tool",
    description: "First",
