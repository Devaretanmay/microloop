...
The [package gallery](https://microloop.dev/packages) displays packages tagged with `pi-package`. Add `video` or `image` fields to show a preview:
...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@microloop/ai`, `@microloop/agent`, `@microloop/coding-agent`, `@microloop/tui`, `typebox`.
