...
**Source:** [`@microloop/tui`](https://github.com/earendil-works/pi-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@microloop/tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `MICROLOOP_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@microloop/tui";
...
Import from `@microloop/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@microloop/tui";
```
...
```typescript
import { matchesKey, Key } from "@microloop/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@microloop/tui";
...
```typescript
import { getMarkdownTheme } from "@microloop/coding-agent";
import { Markdown } from "@microloop/tui";
...
Set `MICROLOOP_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
MICROLOOP_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@microloop/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@microloop/coding-agent";
import { DynamicBorder } from "@microloop/coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@microloop/tui";
...
```typescript
import { BorderedLoader } from "@microloop/coding-agent";
...
For toggling multiple settings. Use `SettingsList` from `@microloop/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@microloop/coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@microloop/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@microloop/coding-agent";
import { matchesKey, truncateToWidth } from "@microloop/tui";
