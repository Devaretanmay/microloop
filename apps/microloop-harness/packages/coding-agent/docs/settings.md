...
| `enableAnalytics` | boolean | `false` | Opt-in analytics data sharing. Currently only asked for during the experimental first-time setup (`MICROLOOP_EXPERIMENTAL=1`) |
...
`enableInstallTelemetry` only controls the anonymous install/update ping to `https://microloop.dev/api/report-install`. Opting out of telemetry does not disable update checks; Pi can still fetch `https://microloop.dev/api/latest-version` to look for the latest version.

Set `MICROLOOP_SKIP_VERSION_CHECK=1` to disable the Pi version update check. Use `--offline` or `MICROLOOP_OFFLINE=1` to disable all startup network operations described here, including update checks, package update checks, and install/update telemetry.
...
When multiple sources specify a session directory, precedence is `--session-dir`, `MICROLOOP_CODING_AGENT_SESSION_DIR`, then `sessionDir` in settings.json.
