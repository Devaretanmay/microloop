...
On WSL, WezTerm may require a visible hardware cursor for IME candidate window positioning. If CJK IME candidates do not follow the text cursor, set `MICROLOOP_HARDWARE_CURSOR=1` before running pi or set `showHardwareCursor` to `true` in settings.
...
If you want the hardware cursor visible, set `MICROLOOP_HARDWARE_CURSOR=1` before running pi (disabled by default for compatibility).
