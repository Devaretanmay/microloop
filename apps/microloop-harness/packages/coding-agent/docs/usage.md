...
|----------|-------------|
| `MICROLOOP_CODING_AGENT_DIR` | Override config directory; default is `~/.pi/agent` |
| `MICROLOOP_CODING_AGENT_SESSION_DIR` | Override session storage directory; overridden by `--session-dir` |
| `MICROLOOP_PACKAGE_DIR` | Override package directory, useful for Nix/Guix store paths |
...
| `MICROLOOP_TELEMETRY` | Override install/update telemetry and provider attribution headers: `1`/`true`/`yes` or `0`/`false`/`no`. This does not disable update checks |
| `MICROLOOP_CACHE_RETENTION` | Set to `long` for extended prompt cache where supported |
