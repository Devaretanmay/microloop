...
- Added `MICROLOOP_DEBUG_REDRAW=1` env var for debugging full redraws (logs triggers to `~/.pi/agent/pi-debug.log`)

### Changed

- Terminal height changes no longer trigger full redraws, reducing flicker on resize
- `clearOnShrink` now defaults to `false` (use `MICROLOOP_CLEAR_ON_SHRINK=1` or `setClearOnShrink(true)` to enable)
...
- Added `MICROLOOP_TUI_WRITE_LOG` environment variable to capture raw ANSI output for debugging
