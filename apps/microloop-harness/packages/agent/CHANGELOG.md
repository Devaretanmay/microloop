...
- Removed the `@microloop/agent/base` selective-provider entrypoint; use the root package with an explicit `Models` instance instead.
...
- Added `@microloop/agent/base` for bundlers that want to pair the agent core with selective `@microloop/ai/base` provider registration ([#5348](https://github.com/earendil-works/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
