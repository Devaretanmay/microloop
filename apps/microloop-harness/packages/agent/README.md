...
Stateful agent with tool execution and event streaming. Built on `@microloop/ai`.

## Installation

```bash
npm install @microloop/agent
```
...
```typescript
declare module "@microloop/agent" {
...
```typescript
import { Agent, streamProxy } from "@microloop/agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@microloop/agent";
