...
```bash
npm install @microloop/ai
```

TypeBox exports are re-exported from `@microloop/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@microloop/ai';
import { builtinModels } from '@microloop/ai/providers/all';
...
```typescript
import { anthropicProvider } from '@microloop/ai/providers/anthropic';
import { openaiProvider } from '@microloop/ai/providers/openai';
import { openrouterProvider } from '@microloop/ai/providers/openrouter';
import { amazonBedrockProvider } from '@microloop/ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@microloop/ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@microloop/ai/providers/all';
...
Stored credentials (API keys entered interactively, OAuth tokens) live in a `CredentialStore` — one type-tagged credential per provider. The library ships an in-memory default; apps inject persistent storage:

```typescript
import { createModels, type CredentialStore } from '@microloop/ai';
...
API-key credentials use the same discriminator as the library's `auth.json` and can carry provider-scoped env/config values:
...
```typescript
import { Type, type Tool, StringEnum } from '@microloop/ai';
...
```typescript
import { validateToolCall, type Tool } from '@microloop/ai';
...
```typescript
import { builtinImagesModels } from '@microloop/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@microloop/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@microloop/ai';
import { openAICompletionsApi } from '@microloop/ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@microloop/ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@microloop/ai/api/openai-responses.lazy';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing standard levels through `high` use provider defaults; `xhigh` and `max` are opt-in and require a non-null map entry. String values are sent to the provider, `null` marks a level unsupported, and maps may skip levels.
...
```typescript
import { stream } from '@microloop/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@microloop/ai/api/<api-id>`.
...
interface OpenAICompletionsCompat {
  ...
  chatTemplateKwargs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort'; omitWhenOff?: boolean }>; // chat_template_kwargs values; use $var for framework-controlled thinking values
...
```typescript
import { createModels, type Context } from '@microloop/ai';
import { anthropicProvider } from '@microloop/ai/providers/anthropic';
import { openaiProvider } from '@microloop/ai/providers/openai';
import { googleProvider } from '@microloop/ai/providers/google';
...
```typescript
import { createModels } from '@microloop/ai';
import { anthropicProvider } from '@microloop/ai/providers/anthropic';
...
- `@microloop/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@microloop/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@microloop/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@microloop/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@microloop/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@microloop/ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@microloop/ai/bedrock-provider';
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for provider auth and configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `MICROLOOP_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
npx @microloop/ai login              # interactive provider selection
npx @microloop/ai login anthropic    # login to specific provider
npx @microloop/ai list               # list available providers
```
...
The legacy flow functions remain available via the `@microloop/ai/oauth` entry point (`loginAnthropic`, `loginOpenAICodex`, `loginGitHubCopilot`, `refreshOAuthToken`, `getOAuthApiKey`); credential storage is the caller's responsibility there. New code should prefer the provider-owned `OAuthAuth` shown above — it composes with the credential store and gets locked auto-refresh for free.
...
```typescript
// Before
import { getModel, complete } from '@microloop/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@microloop/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@microloop/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@microloop/ai`.
